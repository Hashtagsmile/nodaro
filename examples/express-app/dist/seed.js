...
async function seedDemoData() {
    ...
    const db = (0, nodaro_1.getDb)();
    const force = process.env.FORCE_SEED === "1";
    if (!force) {
        const existing = await db.collection("users").estimatedDocumentCount();
        if (existing > 0) {
            console.log("[demo] Seeding skipped — database already has data. Set FORCE_SEED=1 to reseed.");
            return;
        }
    }
    if (force) {
    ...
    const now = () => new Date();
    const u1 = new mongoose_1.default.Types.ObjectId();
    const u2 = new mongoose_1.default.Types.ObjectId();
    const u3 = new mongoose_1.default.Types.ObjectId();
    const u4 = new mongoose_1.default.Types.ObjectId();
    const c1 = new mongoose_1.default.Types.ObjectId();
    const c2 = new mongoose_1.default.Types.ObjectId();
    const c3 = new mongoose_1.default.Types.ObjectId();
    const p1 = new mongoose_1.default.Types.ObjectId();
    const p2 = new mongoose_1.default.Types.ObjectId();
    const p3 = new mongoose_1.default.Types.ObjectId();
    const p4 = new mongoose_1.default.Types.ObjectId();
    const p5 = new mongoose_1.default.Types.ObjectId();
    /** Many top-level fields (Parse-style wide rows) for UI horizontal scroll demos. */
    const wideIdA = new mongoose_1.default.Types.ObjectId();
    const wideIdB = new mongoose_1.default.Types.ObjectId();
    ...
    console.log("[demo] Seeded: users, categories, products, orders, wide_records (wide table demo).");
}
